...
if (($action == 'add_record' || $action == "edit_record") && ! empty($_POST)) {
...
{
	$record = isset($_POST['record']['recorde']) ? trim($_POST['record']['recorde']) : null;
	$type = isset($_POST['record']['typee']) ? $_POST['record']['typee'] : 'A';
	$prio = isset($_POST['record']['prioe']) ? (int) $_POST['record']['prioe'] : null;
	$content = isset($_POST['record']['contente']) ? trim($_POST['record']['contente']) : null;
	$ttl = isset($_POST['record']['ttle']) ? (int) $_POST['record']['ttle'] : 18000;
}
	...
	if ($action == 'add_record')
{
	...
	if (empty($errors)) {
		...
		// success message (inline)
		$success_message = $lng['success']['dns_record_edited'];
